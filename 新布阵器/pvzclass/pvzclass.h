...
4.生成限定机器码与有效期的布阵码：使用流程如下\n\
	(1)裁判从玩家除拿到多个机器码与布阵时间 \n\
	(2)生成加密布阵码后，粘贴发给玩家即可\n\
5.比赛模式布阵：额外开启反作弊检测；也会提示主题序号；使用流程如下\n\
	(1)拿到加密布阵码输入布阵器；\n\
	(2)输入密钥解密布阵码；\n\
	后正常游戏\n\
6.残局布阵: 限时30min，一共15关；\n\
	(1) 游戏内的restart和lose会变成跳到下一关；\n\
	(2) 玩法：每关阳光随机，只需要尽可能拿到更多脑子即可；如若相同，比时间；\n\
7. 生成随机的15关布阵码:  \n\
	(1)主题分布原版冲关完全一致；\n\
	(2)花数分布：等概率随机1-3\n\
	(4)阳光：随机500-800\n\
8.锁主题锁花数，带单关计时，其实8应该是娱乐模式：什么消消乐模式（，考验眼力和反应力\n\
9.不限时冲关：与正常冲关无异，旨在提供一些数据便于玩家后续复盘；\n\
	(1)快捷键：a)shift+A 切换自动收集 2)shift+D 切换加速，每关开始会恢复原速 3)shift+Q 强制退出 4)shift+J 跳关，用于卡礼物然后无法过关的情况 5)shift+M 切换禁用女仆\n\
a.导出本关ize阵型代码：方便捏码\n\
b.单关布阵";
