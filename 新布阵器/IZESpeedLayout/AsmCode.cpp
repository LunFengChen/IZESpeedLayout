...
void AsmCode::asm_init() {
	length = 0;
	if (code != nullptr) {
		delete[] code;
	}
	code = new unsigned char[4096 * 256];
}

void AsmCode::asm_add_byte(unsigned char value) {
	code[length] = value;
	length += 1;
}

void AsmCode::asm_add_word(unsigned short value) {
	(unsigned short&)code[length] = value;
	length += 2;
}

void AsmCode::asm_add_dword(unsigned int value) {
	(unsigned int&)code[length] = value;
	length += 4;
}

void AsmCode::asm_add_list(std::initializer_list<unsigned char> value) {
	for (auto it = value.begin(); it != value.end(); it++)
		asm_add_byte(*it);
}

void AsmCode::asm_push(int value) {
	//wcout << "现在正在调用code.cpp的asm_push，执行成功" << endl;

	asm_add_byte((unsigned char)(0x68));
	asm_add_dword(value);
}

void AsmCode::asm_mov_exx(AsmCode::Reg reg, int value) {
	unsigned char mov_exx[] = { 0xb8, 0xbb, 0xb9, 0xba, 0xbe, 0xbf, 0xbd, 0xbc };
	asm_add_byte(mov_exx[static_cast<unsigned int>(reg)]);
	asm_add_dword(value);
}

void AsmCode::asm_add_exx(AsmCode::Reg reg, int value) {
	unsigned char add_exx[] = { 0x05, 0xc3, 0xc1, 0xc2, 0xc6, 0xc7, 0xc5, 0xc4 };
	if (reg != AsmCode::Reg::EAX)
		asm_add_byte((unsigned char)(0x81));
	asm_add_byte(add_exx[static_cast<unsigned int>(reg)]);
	asm_add_dword(value);
}

void AsmCode::asm_mov_exx_dword_ptr(AsmCode::Reg reg, int value) {
...
void AsmCode::asm_mov_exx_dword_ptr_exx_add(AsmCode::Reg reg, int value) {
...
void AsmCode::asm_ret() {
	asm_add_byte((unsigned char)(0xc3));
}

void AsmCode::asm_code_inject(HANDLE handle) {
